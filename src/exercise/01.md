...
- The module must be exported in a default React Component, like a Wrapper.
- It should be imported with React Lazy, and rendered between React suspenses to render a fallback while loading
- Webpack, any bundler or the browser by itself caches the imports, so it doesn't matter if you use 3000 imports, it will return the same value
